              ...
              return (
                <div key={license.id} className="border rounded-2xl p-4 flex flex-col relative bg-white shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span 
                      className={`text-sm font-medium ${expired ? 'flex items-center gap-1' : 'text-gray-700'}`}
                      style={expired ? { color: '#ca8a04' } : undefined}
                    >
                      {expired && <AlertTriangle className="w-4 h-4" style={{ color: '#ca8a04' }} />}
                      {license.expiryDate}
                    </span>
                  </div>
                  {expired && (
                    <div className="text-xs font-medium mb-1 mt-1" style={{ color: '#ca8a04' }}>
